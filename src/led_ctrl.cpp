...
void LEDIndicate::Done(void) {
  ...
  LEDPriorityDispatch(led_stack.led_element[led_stack.top].priority, LED_OFF, 0,
                      0, 0, 0, 0);
  RunStackTop();
  xSemaphoreGive(ledxSemaphore);
}

void LEDIndicate::Set(LEDPriority priority, LEDOnStatus on_status, uint16_t R,
                      uint16_t G, uint16_t B, uint32_t onTimeMS,
                      uint32_t offTimeMS, uint16_t count) {
  xSemaphoreTake(ledxSemaphore, pdMS_TO_TICKS(1000));
  LED_Element top_elem =
      led_stack.top < 0 ? LED_Element{} : led_stack.led_element[led_stack.top];

  LEDPriorityDispatch(priority, on_status, R, G, B, onTimeMS, offTimeMS, count);

  if (top_elem == LED_Element{} || led_stack.top < 0 ||
      top_elem != led_stack.led_element[led_stack.top]) {
    RunStackTop();
  }
...
void LEDIndicate::LEDPriorityDispatch(LEDPriority priority,
                                      LEDOnStatus on_status, uint16_t R,
                                      uint16_t G, uint16_t B, uint32_t onTimeMS,
                                      uint32_t offTimeMS, uint16_t count) {
  switch (on_status) {
  case LED_ON:
...
void LEDIndicate::LEDHandleOnBlink(LEDPriority priority, LEDOnStatus on_status,
  ...
  if (led_stack.top < 0) {
    ...
    uint8_t i = 0;
    for (; i < MAX_NUMBER_OF_LED_PRIORITY && i <= led_stack.top &&
    ...
    } else if (i < MAX_NUMBER_OF_LED_PRIORITY) {
...
void LEDIndicate::LEDHandleOff(LEDPriority priority, LEDOnStatus on_status) {
  if (led_stack.top < 0) {
    return;
  }
  uint8_t i = 0;
  while (i < MAX_NUMBER_OF_LED_PRIORITY && i <= led_stack.top &&
         led_stack.led_element[i].priority != priority)
    i++;
  if (i == MAX_NUMBER_OF_LED_PRIORITY || i > led_stack.top) {
    return;
  }
...
void LEDIndicate::RunStackTop(void) {
  ...
  if (CheckIsMask(led_stack.led_element[led_stack.top].priority)) {
    LEDPriorityDispatch(led_stack.led_element[led_stack.top].priority, LED_OFF,
                        0, 0, 0, 0, 0);
    RunStackTop();
  }
  ...
  case LED_ON:
    mled.Set(true, led_stack.led_element[led_stack.top].R,
             led_stack.led_element[led_stack.top].G,
             led_stack.led_element[led_stack.top].B);
    break;

  case LED_Blink:
  case LED_Count_Blink:
    mled.Blink(led_stack.led_element[led_stack.top].onTimeMS,
               led_stack.led_element[led_stack.top].offTimeMS,
               led_stack.led_element[led_stack.top].R,
               led_stack.led_element[led_stack.top].G,
               led_stack.led_element[led_stack.top].B,
               led_stack.led_element[led_stack.top].blink_count);
    break;
...
void LEDIndicate::SetMask(LEDPriority priority) {
  xSemaphoreTake(ledxSemaphore, pdMS_TO_TICKS(1000));
...
void LEDIndicate::CancelMask(LEDPriority priority) {
  xSemaphoreTake(ledxSemaphore, pdMS_TO_TICKS(1000));
  LedMask[priority] = false;
  xSemaphoreGive(ledxSemaphore);
}

bool LEDIndicate::CheckIsMask(LEDPriority priority) {
  return LedMask[priority];
}

LEDIndicate::LEDIndicate(LEDIndicateCallback callback) {
  IsStop = false;
  mled.Init(this, callback);
  led_stack.top = -1;
